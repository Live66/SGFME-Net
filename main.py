...
def perform_saliency(rgb_image):
    superpixels = apply_slic(200,rgb_image)
...
if __name__=='__main__':
    # filename = "./boat.jpg"#G:\1.course\myproject\Saliency-Extraction-master\Test_Data\1.png ./image_dataset/DUT-OMRON-image/DUT-OMRON-image/im005.jpg
    # filename = "./Test_Data/8.jpg"
    filename = "./00771.jpg"
    # b = cv2.split(filename)[0]
    # g = cv2.split(filename)[1]
    # r = cv2.split(filename)[2]
    # filename = cv2.merge([r, g, b])
    # filename = filename.convert("RGB")  # 4通道转化为rgb三通道


    #filename_a = str(argv[1])
