...
from my_dataset import DIORVOCDataSet
...
def create_model(num_classes):
    ...
    # --- resnet50_large fpn backbone --- #
    # backbone = torchvision.models.resnet101(pretrained=True)
    # # print(backbone)
    # return_layers = {"layer1": "0",  # stride 8
    #                  "layer2": "1",  # stride 16
    #                  "layer4": "2"}  # stride 32
    # # 提供给fpn的每个特征层channel
    # in_channels_list = [256, 512, 2048]
    # new_backbone = create_feature_extractor(backbone, return_layers)
    # img = torch.randn(1, 3, 224, 224)
    # outputs = new_backbone(img)
    # [print(f"{k} shape: {v.shape}") for k, v in outputs.items()]
    ...
    # # print(backbone)
    # return_layers = {"features.1": "0",  # stride 8
    #                  "features.5": "1",  # stride 16
    #                  "features.6": "2"}  # stride 32
    # # 提供给fpn的每个特征层channel
    # in_channels_list = [16, 112, 192]
    # new_backbone = create_feature_extractor(backbone, return_layers)
    # img = torch.randn(1, 3, 224, 224)
    # outputs = new_backbone(img)
    # [print(f"{k} shape: {v.shape}") for k, v in outputs.items()]
    ...
    aspect_ratios = ((0.3,  0.5, 1.0, 2.0, 3.0),) * len(anchor_sizes)
...
def main(args):
    ...
    train_dataset = DIORVOCDataSet(VOC_root, data_transform["train"], True)
    train_sampler = None
    ...
    val_dataset = DIORVOCDataSet(VOC_root, data_transform["val"], False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        torch.save(save_files, "./save_weights/31resNetFpn-model-{}.pth".format(epoch))
